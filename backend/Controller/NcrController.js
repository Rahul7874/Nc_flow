...
export const PatchBYidncr = expressAsyncHandler(async (req, res) => {
    let {id,Id,Type,Problem,ProcessStage,PartNo,ReworkHrs,Issue,FailureType,RCA,Resolutionowner,ResolutionownerId,RCAValidator,RCAValidatorId,Finalapprover,FinalapproverId} = req.body.data
    try {
        if (id) {
            const data = await Ncr.findByIdAndUpdate(id, {Id,Type,Problem,ProcessStage,PartNo,ReworkHrs,Issue,FailureType,RCA,Resolutionowner,ResolutionownerId,RCAValidator,RCAValidatorId,Finalapprover,FinalapproverId}, { new: true })
